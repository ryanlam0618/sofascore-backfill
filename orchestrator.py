...
# ── Load SOFA_PROXY from .env if not already set ────────────────────────────────
_env_path = Path(__file__).parent.parent / ".env"
if not os.environ.get("SOFA_PROXY") and _env_path.exists():
    for line in _env_path.read_text().splitlines():
        line = line.strip()
        if line.startswith("SOFA_PROXY=") and not line.startswith("SOFA_PROXY_HOST"):
            _, _, val = line.partition("=")
            os.environ["SOFA_PROXY"] = val.strip()
            break
...
def run_script_sequential(
    ...
    for attempt in range(MAX_RETRIES + 1):
        ...
        try:
            env = {**os.environ}  # inherit full env including SOFA_PROXY if set
            with open(log_path, "a") as lf:
