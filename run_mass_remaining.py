#!/usr/bin/env python3
"""
Run only the 5 remaining cup competitions for mass test.
Resumes from existing data/cloak_test_mass_results.json.
"""
...
from rotate_proxy import ProxyManager
...
# Only run these 5 cups that are missing/incomplete
TARGET_COMPS = ["DFB Pokal", "J.League Cup", "Emperor's Cup", "Australia Cup", "Chinese FA Cup"]
...
def run():
    ...
    for comp_name in TARGET_COMPS:
        if comp_name not in season_map:
            print(f"  {comp_name} — not in season map")
            continue
    ...
    print(f"\n{'='*60}")
    print("MASS TEST RESUME FINAL SUMMARY")
    print(f"{'='*60}")
    print(f"Seasons tested in this run: {total_seasons}")
    print(f"Total entries in JSON:     {len(detail_results)}")
