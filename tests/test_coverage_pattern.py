#!/usr/bin/env python3
"""
Coverage pattern test: 24 competitions × 10 seasons × 10 events per season.
For each event, record coverage field, lineups status, shotmap status.
"""
...
print("=== Coverage Pattern Test: 10 events per season ===")
print(f"[{time.strftime('%H:%M:%S')}] Launching CloakBrowser...")
...
def get_events_from_round(ut_id, season_id, round_n):
    """Get event IDs from a specific round."""
...
def check_event(eid):
    """Check coverage, lineups, shotmap for one event. Returns dict."""
...
for comp_name, comp_info in season_map.items():
    ...
    for year_label, season_id in sorted_seasons:
        ...
        if season_mode in ("national", "uefa"):
            # League: try rounds 1-5 to get 10 events
            for r in range(1, 8):
        ...
        with open("data/cloak_coverage_pattern_10x10.json", "w") as f:
...
# Save (already saved incrementally, but final save)
with open("data/cloak_coverage_pattern_10x10.json", "w") as f:
    json.dump(all_results, f, indent=2, ensure_ascii=False)
...
# Print summary table
print(f"\n{'='*70}")
print("SUMMARY: Coverage pattern per competition per season")
print(f"{'='*70}")
print(f"{'Competition':<22} {'Season':<8} {'N':>3} {'cov=1':>5} {'cov=-1':>6} {'LU✅':>4} {'LU❌':>4} {'SM✅':>4} {'SM❌':>4}")
print("-" * 70)
for comp, seasons in all_results.items():
    for season, events in seasons.items():
        if not events:
            continue
        n = len(events)
        c1 = sum(1 for e in events if e.get("coverage") == 1)
        cn1 = sum(1 for e in events if e.get("coverage") == -1)
        lu_ok = sum(1 for e in events if e.get("lineups") == 200)
        lu_no = sum(1 for e in events if e.get("lineups") == 404)
        sm_ok = sum(1 for e in events if e.get("shotmap") == 200)
        sm_no = sum(1 for e in events if e.get("shotmap") == 404)
        print(f"{comp:<22} {season:<8} {n:>3} {c1:>5} {cn1:>6} {lu_ok:>4} {lu_no:>4} {sm_ok:>4} {sm_no:>4}")
